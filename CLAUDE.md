...
Reading Room is a personal catalogue of focused paper breakdowns. **You** — Claude Code, run interactively by the reader under their own subscription; never the Anthropic API or `claude -p` — write every report. `scripts/build.py` renders them into a static site the reader opens in the app or hosts on GitHub Pages.

## What the reader asks you to do

The app sends these into this terminal (usually with `--approve`); the reader may also type them. Each one is fully specified in `.claude/commands/<name>.md` — read that file when the command is invoked.
...
- Read PDFs with the Read tool directly (it renders pages). Don't leave `.txt` files next to the PDFs in `papers/` — extracted text goes in `.cache/` (gitignored).
...
.claude/commands/*.md            the workflows above           .claude/skills/     verify-build, run-reading-room
```
