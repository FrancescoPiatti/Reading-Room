---
name: learn
description: "Discuss an already-catalogued paper interactively, then on End chat / \"done\" compact the conversation into a standalone Discussion stored like a comparison (listed below the reading list). Invoke only when the user explicitly selects $learn or /learn."
---
...
Run `python3 scripts/verify.py --build`, fix any `✗` failures, confirm the new chat appears under **Discussions** on `docs/index.html` (and as **"Discussed in"** on the paper's report), and tell the reader to refresh. Mention anything left open.
