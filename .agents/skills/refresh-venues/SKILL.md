---
name: refresh-venues
description: "Check every catalogued paper still marked as an arXiv preprint to see whether it has since been published, and update its venue + BibTeX if so. Invoke only when the user explicitly selects $refresh-venues or /refresh-venues."
---
...
For each candidate, check whether a peer-reviewed version now exists, using the running assistant's web connector against **DBLP** — the same source the other commands use:
