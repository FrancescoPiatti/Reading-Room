...
Reading Room is a personal catalogue of focused paper breakdowns. **You** — the assistant reading this (Codex CLI), run interactively by the reader under their own subscription; never an external assistant API or a non-interactive assistant CLI — write every report. `scripts/build.py` renders them into a static site the reader opens in the app or hosts on GitHub Pages.

## Native workflow invocation

- Codex-native explicit invocations are `$explain-paper`, `$compare`, `$learn`, `$deep-dive`, `$remove`, `$refresh-venues`, `$setup`, `$backup` and `$update`; arguments follow the skill mention as normal text.
- The app may send the equivalent slash forms (`/explain-paper …`). Treat each slash form as the same workflow and preserve every option, path, schema and approval gate.
- The command-style skills are explicit-only; the reusable `$verify-build` and `$run-reading-room` skills may also trigger when their descriptions match.

## What the reader asks you to do

The app sends these into this terminal (usually with `--approve`); the reader may also type them. Each one is fully specified in `.agents/skills/<name>/SKILL.md` — read that file when the command is invoked.
...
- Read PDFs directly with your PDF-capable file reader (it renders pages). Don't leave `.txt` files next to the PDFs in `papers/` — extracted text goes in `.cache/` (gitignored).
...
.agents/skills/<name>/SKILL.md   the workflows above (+ verify-build, run-reading-room)
```
