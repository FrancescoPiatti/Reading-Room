---
description: Read a paper (arXiv id, URL, or local PDF path) and produce a focused HTML report in the Reading Room catalogue.
argument-hint: <arxiv-id | arxiv-url | path/to/paper.pdf> [--views a,b,c] [--depth skim|standard|deep] [--audience peer|newcomer] [--focus "..."] [--approve]
---

You are building one entry in a personal "Reading Room" — a catalogue of focused paper breakdowns. Work **interactively** and stop at the approval gate below. Do **not** use any external API or CLI commands directly; this runs inside the current interactive session.
...
Then **read the PDF** with the Read tool directly — it renders pages so you can see equations, figures, and tables. Read the whole thing, appendices included if present. Only if rendering is unavailable, fall back to a text extraction — and write it to `.cache/<id>.txt` (gitignored), **never** beside the PDF in `papers/`. Don't leave `.txt` files in `papers/`.
...
- For the canonical entry, fetch it with the web connector (WebFetch/WebSearch) — DBLP exposes a stable `.bib` per paper (e.g. `https://dblp.org/rec/<key>.bib`). Set it as a raw `"bibtex": "@inproceedings{...}"` string — it's used verbatim and wins over `published`. Don't hand-fabricate page numbers / volume; if unsure, use the structured `published` fields you're confident about and leave the rest out.
...
Run `python3 scripts/verify.py --build` (builds, then runs QA). Fix any `✗` failures before continuing — only warnings are acceptable. Confirm `reports/<id>/digest.json` and `docs/papers/<id>/index.html` were written, and tell the user to open `docs/index.html` (or refresh it). Mention anything you skipped or were unsure about.
