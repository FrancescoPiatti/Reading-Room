...
Run `python verify.py --build` (builds, then runs QA). Fix any `✗` failures before continuing — only warnings are acceptable. Confirm `reports/<id>/digest.json` and `docs/papers/<id>/index.html` were written, and tell the user to open `docs/index.html` (or refresh it). Mention anything you skipped or were unsure about.
