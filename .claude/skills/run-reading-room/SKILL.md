...
```bash
node .claude/skills/run-reading-room/driver.mjs serve            # start headless; prints http://127.0.0.1:<port>/
node .claude/skills/run-reading-room/driver.mjs smoke            # 5 health+security checks (exit 0 = all pass)
node .claude/skills/run-reading-room/driver.mjs term 'echo hi'   # run a shell command in the app's shared pty
node .claude/skills/run-reading-room/driver.mjs shot papers/hornik1989/ out.png 8000   # CDP screenshot (waitMs last)
node .claude/skills/run-reading-room/driver.mjs shot / out.png                          # catalogue
node .claude/skills/run-reading-room/driver.mjs build            # python3 scripts/verify.py --build (build + QA)
node .claude/skills/run-reading-room/driver.mjs stop
```
