...
You are adding an **incremental deep dive** (a full worked derivation, proof walkthrough, or mechanism breakdown) to an existing Reading Room report. This does NOT regenerate the digest — it either **appends** a self-contained entry to the paper's `deepdives` array (its own **Deep dive** tab) **or merges** into a digest section you name, then rebuilds. Work **interactively**; do **not** use the Anthropic API or `claude -p`.
